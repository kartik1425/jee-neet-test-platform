...
import { ArrowUpRight, CheckCircle2, ChevronRight, Clock, FileText, History, XCircle } from "lucide-react";
    ...
    <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
      ...
      {recentAttempts.length === 0 ? (
        <div className="py-10 text-center rounded-2xl bg-slate-50 border border-dashed border-slate-200 text-xs text-slate-500 space-y-2">
          ...
          <p className="font-semibold text-slate-700">No test attempts completed yet</p>
                  ...
                  <td className="py-4 pl-2 font-medium text-slate-900">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`inline-flex items-center px-2 py-0.2 rounded-full text-[10px] font-bold border ${getExamBadgeColor(
                            attempt.examType
                          )}`}
                        >
