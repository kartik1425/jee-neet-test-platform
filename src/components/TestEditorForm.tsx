...
import { MediaQuestionExtractorModal } from "@/components/tests/MediaQuestionExtractorModal";
import {
...
export function TestEditorForm({ test, availableQuestions }: TestEditorFormProps) {
  ...
  // Multimodal AI Extractor Modal State
  ...
  const getJoinUrl = () => {
    if (typeof window !== "undefined") {
      return `${window.location.origin}/exam/join/${test.id}`;
    }
    return `/exam/join/${test.id}`;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold text-xs rounded-xl shadow transition"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
          ...
          {isEditable && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold text-xs rounded-xl shadow transition"
              >
                <Sparkles className="w-3.5 h-3.5" /> AI Extract (PDF / Photo)
              </button>

              <button
                type="button"
                onClick={() => setIsAddModalOpen(true)}
                className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs rounded-xl shadow transition"
              >
        ...
        {test.questions.length === 0 ? (
          <div className="p-8 text-center border border-dashed border-slate-300 rounded-2xl space-y-4 bg-slate-50/50">
            <div className="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center mx-auto">
              <BookOpen className="w-6 h-6" />
            </div>
            <div className="space-y-1">
              <p className="text-sm font-bold text-slate-800">No questions added to this test yet</p>
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Choose questions from the verified question bank or use AI Multimodal OCR to extract questions from PDFs or camera photos.
              </p>
            </div>
            {isEditable && (
              <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setIsExtractorModalOpen(true)}
                  className="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold text-xs rounded-xl shadow transition"
                >
                  <Sparkles className="w-4 h-4" /> AI Multimodal Extractor (PDF / Photo)
                </button>
                <button
                  type="button"
                  onClick={() => setIsAddModalOpen(true)}
                  className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold text-xs rounded-xl shadow-sm transition"
                >
                  <Plus className="w-4 h-4" /> Select from Question Bank
                </button>
              </div>
            )}
          </div>
      ...
      {/* AI Multimodal Question Extractor Modal */}
      <MediaQuestionExtractorModal
        testId={test.id}
        examType={test.exam_type}
        isOpen={isExtractorModalOpen}
        onClose={() => setIsExtractorModalOpen(false)}
        onSuccess={(count) => {
          setSuccessMsg(`Successfully added ${count} AI-extracted questions to the test!`);
          router.refresh();
        }}
      />
      ...
      {isShareModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 space-y-5 animate-in fade-in duration-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-900">Share Test Invite Link</h3>
                  <p className="text-[11px] text-slate-500">Instant access URL for students</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  readOnly
                  value={getJoinUrl()}
                  className="flex-1 px-3.5 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl font-mono text-slate-700 select-all focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button
                  type="button"
                  onClick={handleCopyLink}
                  className={`inline-flex items-center gap-1.5 px-4 py-2.5 font-bold text-xs rounded-xl shadow transition ${
                    copiedLink
            ...
            <div className="p-4 rounded-2xl bg-blue-50/70 border border-blue-100 text-xs text-blue-900 space-y-2">
              <span className="font-bold flex items-center gap-1.5 text-blue-950">
                <CheckCircle2 className="w-4 h-4 text-blue-600" /> Smart Onboarding Included:
              </span>
              <ul className="list-disc list-inside space-y-1 text-blue-800 text-[11px]">
                <li><strong>Logged-in Students:</strong> Land directly into the test lobby ready to click &quot;Start Exam&quot;.</li>
                <li><strong>New / Logged-out Students:</strong> Prompted to quickly log in or sign up, and automatically redirected straight into this test.</li>
                <li><strong>Individual Timers:</strong> Full duration timer begins individually when each student starts.</li>
              </ul>
            </div>

            <div className="flex items-center justify-between pt-2">
              <Link
                href={`/exam/join/${test.id}`}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-800 transition"
              >
                <ExternalLink className="w-3.5 h-3.5" /> Open student view in new tab
              </Link>
              <button
                type="button"
                onClick={() => setIsShareModalOpen(false)}
